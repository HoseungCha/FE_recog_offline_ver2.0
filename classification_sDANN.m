...
%-------------------------------------------------------------------------%
% written by Ho-Seung Cha, Ph.D Student,
...
DB_t = get_DB_prime(DB,DB_c,'T',2,'fhandle',str2func('transWithDPW'));
...
%     a = setlabtype(a,'soft');
    % W1 = ldc(a);
%     W = a*ldc*classc;
%     mdl = fitcdiscr(xtr,ytr,'DiscrimType','quadratic');
%     rng(1)
...
%----------------------------------------------------------%
    ...
    target(:,:,u,n2,n_t+1,count) = yte;
    output(:,:,u,n2,n_t+1,count) = ypr_cv;
    %=====================================================%
end      
end
end
