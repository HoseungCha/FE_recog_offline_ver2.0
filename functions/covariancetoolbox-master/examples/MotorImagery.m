...
data.idxValidation = 289:300;
data.idxTest = 401:576;
...
COVval = data.data(:,:,data.idxValidation);
Yval   = data.labels(data.idxValidation);
...
for i=1:4
    for j=i+1:4
        ...
        ixval = (Yval==i)|(Yval==j);
        % Classification
        Ytest = tslda(COVtest(:,:,ixtest),COVtrain(:,:,ixtrain),...
            Ytrain(ixtrain),COVval(:,:,ixval),metric_mean,update);
        % Accuracy
        acc(i,j) = 100*mean(Ytest==trueYtest(ixtest));
    end
end
