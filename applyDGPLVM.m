...
addpath(genpath('E:\OneDrive\연구지식\Gaussian Process Latent Variable Model\dgplvm-master'));
...
% yt = getSimplex(get_labels(N1,M,1),M)';
...
gscatter(model.X(:,1),model.X(:,2),y);

kfoldLoss(crossval(fitcdiscr(model.X,y)))

kfoldLoss(crossval(fitcdiscr(x,y)))
